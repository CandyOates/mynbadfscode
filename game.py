...
from utils import _api_scrape, _get_json
...
class PlayerMovements:
    def __init__(self,
                 game_id,
                 event_id):
        url = 'http://stats.nba.com/stats/locations_getmoments?gameid=%s&eventid=%s' % (str(game_id),str(event_id))
        import urllib2
        page = urllib2.urlopen(url)
        text = page.read().replace('null','None').replace('true','True').replace('false','False')
        self.json = eval(text)
